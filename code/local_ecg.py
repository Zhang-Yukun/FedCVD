...
if __name__ == "__main__":
    ...
    for idx in range(1, 5):
        train_dataset = get_dataset(
            [
                os.path.join("/data/zyk/data/dataset/ECG/preprocessed/client" + str(idx) + "/train_valid_20_r.csv")
            ],
            base_path="/data/zyk/data/dataset/ECG/preprocessed",
            locations=["client" + str(idx)],
            file_name="records_20.h5",
            n_classes=20
        )
        base_path = "/data/zyk/code/fedmace_benchmark/output/local/client" + str(idx) + "/"
        for batch_size in [32]:
            for lr in [0.1, 0.01, 0.001, 0.0001, 0.00001]:
                ...
                logger = Logger(log_name="client" + str(idx), log_file=output_path + "logger.log")
