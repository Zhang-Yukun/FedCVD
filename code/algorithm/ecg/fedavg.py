...
class FedAvgNonIIDServerHandler(SyncServerHandler):
    """FedAvg server handler."""
    def __init__(
        self,
        model: torch.nn.Module,
        ...
        super(FedAvgNonIIDServerHandler, self).__init__(model, test_loaders, criterion, output_path, evaluator, communication_round, num_clients, sample_ratio, device, logger)
        guarantee_path(self.output_path + "server/train/")
        guarantee_path(self.output_path + "server/local_test/")
        guarantee_path(self.output_path + "server/global_test/")
...
class FedAvgNonIIDSerialClientTrainer(SGDSerialClientTrainer):
    def __init__(
            self,
            model,
            num_clients,
        ...
        super(FedAvgNonIIDSerialClientTrainer, self).__init__(model, num_clients, train_loaders, test_loaders, device, logger, personal)
        self._model = deepcopy(model).to(self._device)
        self.lr = lr
        self.criterion = criterion
        self.max_epoch = max_epoch
