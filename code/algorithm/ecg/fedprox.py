...
class FedProxNonIIDServerHandler(FedAvgNonIIDServerHandler):
    def global_update(self, buffer):
        parameters_list = [ele[0] for ele in buffer]
        serialized_parameters = Aggregators.fedavg_aggregate(parameters_list, None)
        SerializationTool.deserialize_model(self._model, serialized_parameters)
...
class FedProxNonIIDSerialClientTrainer(FedAvgNonIIDSerialClientTrainer):
    def __init__(
            self,
            mu,
            model,
        ...
        super(FedProxNonIIDSerialClientTrainer, self).__init__(
            model=model,
            num_clients=num_clients,
            train_loaders=train_loaders,
            test_loaders=test_loaders,
