...
if __name__ == "__main__":
    ...
    train_dataset = get_echo_dataset(
        [
            os.path.join(f"/data/zyk/data/dataset/ECHO/preprocessed/client{args.client_idx}/train.csv")
        ],
        base_path="/data/zyk/data/dataset/ECHO/preprocessed",
        locations=[f"client{args.client_idx}"],
    ...
    base_path = f"/data/zyk/code/fedmace_benchmark/output/echo/balance/client{args.client_idx}/{args.model}/seed/"
    ...
    logger = Logger(log_name=f"client{args.client_idx}", log_file=output_path + "logger.log")
    ...
    if args.model == "unet":
        ...
        trainer = CentralizedSGDResNetTrainer(
            model=model,
            train_loader=train_loader,
            test_loaders=test_loaders,
            lr=lr,
